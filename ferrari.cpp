...
{
    double r = hypot(x.real(), x.imag()); //форма
    if (r > 0.0)
    {
        double a = atan2(x.imag(), x.real()); //Средний
        n = 1.0 / n;
        r = pow(r, n);
        a *= n;
...
int main()
{
    ...
    x[4] = Ferrari(x, a, b, c,d,e); // Проверка метода Ferrari
    std::cout << "root1: " << x[0].real() << std::endl << "root2: " << x[1].real() << std::endl << "root3: " << x[2].real() << std::endl << "root4: " << x[3].real() << std::endl;
